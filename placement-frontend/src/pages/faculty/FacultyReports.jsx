...
import { Filter, TrendingUp, Users, Award, Briefcase } from 'lucide-react';
  ...
  return (
    <FacultyLayout>
      <div className="space-y-6">
        <div>
          <h1 className="text-2xl font-bold text-slate-800">Dynamic Reports</h1>
          <p className="text-slate-500 text-sm mt-1">Real-time placement and internship analytics</p>
        </div>

        {/* Filters */}
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5">
          <p className="text-xs font-semibold uppercase tracking-wider text-slate-400 mb-4 flex items-center gap-2"><Filter size={12} /> Filters</p>
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
            {[
              { label: 'Year', field: 'year', options: [['', 'All Years'], ['1','Year 1'], ['2','Year 2'], ['3','Year 3'], ['4','Year 4']] },
              ...
              { label: 'Organization', field: 'organization_id', options: [['', 'All'], ...orgs.map(o => [String(o.organization_id), o.organization_name])] },
              ...
              <div key={field}>
                ...
                <select
                  ...
                  className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl text-sm text-slate-700 focus:outline-none focus:border-purple-500 focus:ring-1 focus:ring-purple-500 transition-all"
                >
          ...
          <div className="flex gap-2 mt-4">
            <button onClick={() => loadReport(filters)} className="flex items-center gap-2 bg-purple-600 hover:bg-purple-500 text-white text-sm font-semibold px-4 py-2 rounded-xl transition-all">
              <Filter size={14} /> Apply Filters
            </button>
            <button onClick={() => { const empty = { year:'',cgpa_min:'',cgpa_max:'',organization_id:'',type:'' }; setFilters(empty); loadReport(empty); }}
              className="px-4 py-2 bg-slate-100 hover:bg-slate-200 text-slate-600 text-sm font-medium rounded-xl transition-all">
              Clear
            </button>
          </div>
        </div>
          ...
          <>
            ...
            {report.summary && (
              <div className="grid grid-cols-3 gap-4">
                {[
                  { label: 'Students Placed', value: report.summary.totalPlaced || 0, color: 'purple' },
                  { label: 'Internships', value: report.summary.totalInternships || 0, color: 'blue' },
                  { label: 'Placements', value: report.summary.totalPlacements || 0, color: 'green' },
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
              ...
              {report.placementByOrganization?.length > 0 && (
                <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5">
                  <h3 className="font-semibold text-slate-800 mb-4">Top Organizations</h3>
                  <ResponsiveContainer width="100%" height={220}>
              ...
              {report.placementByYear?.length > 0 && (
                <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5">
                  <h3 className="font-semibold text-slate-800 mb-4">Placements by Year</h3>
                  <ResponsiveContainer width="100%" height={220}>
